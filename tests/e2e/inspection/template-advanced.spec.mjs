...
test.describe("Inspection template JSON export", () => {
  ...
  test("preserves pdf_include in the JSON downloaded through the UI", async ({ page }) => {
    ...
    try {
      const update = await page.request.put(`/inspection/api/sheet/template/${firstCategory.id}`, {
      ...
      const restore = await page.request.put(`/inspection/api/sheet/template/${firstCategory.id}`, {
