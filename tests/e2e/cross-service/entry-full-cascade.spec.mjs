...
// clears every entry for a year, writing a tombstone per removed team. There is no
// synchronous fan-out anymore — each downstream pulls entry's versioned team-state
// snapshot (SSE-triggered) and its enforcement cascades the tombstones (keyed by
// immutable team_id) on the next sync. We seed an ISOLATED non-current year with our
// own nums/emails so we never touch shared seed data (the current-year tables other
// shards assert against) and can safely wipe the whole year.
//
// Queue registration only works for the current year, so this spec asserts the
// documents side of the cascade (queue never caches a year it has no data for, so
// the tombstones are simply irrelevant to it here).
...
test.describe("Entry bulk deletion fans out cascade to documents", () => {
  ...
  test.afterAll(async () => {
    // Idempotent cleanup: wipe the isolated year's entries again (tombstones are
    // idempotent for downstream syncs), then drop any leftover documents mappings.
  ...
  test("deleting all entries for a year removes the teams from documents", async () => {
    ...
    // 1b. Entry rows now expose the immutable team id (the key downstream
    //     services converge on) alongside the mutable num.
    const seededRes = await fetch(`${BASE_URL}/entry/api/entries?year=${ISO_YEAR}`, {
      headers: adminHeaders,
    });
    expect(seededRes.status).toBe(200);
    const seeded = await seededRes.json();
    for (const num of NUMS) {
      expect(seeded[num]).toBeTruthy();
      expect(Number.isInteger(seeded[num].id)).toBeTruthy();
    }
    ...
    // 3. Wait until documents' team-state sync has resolved every mapping to its
    //    team's immutable id. The tombstone cascade deletes by team_id, so a
    //    mapping still carrying NULL team_id (created while documents' cache for
    //    this year was cold) would survive the bulk deletion.
    await expect.poll(async () => {
      ...
      return NUMS.every((num) => data.find((m) => m.team_num === num)?.team_id != null);
    }, { timeout: 15_000 }).toBeTruthy();

    // 4. Bulk-delete every entry for the isolated year → one tombstone per team.
    const delRes = await deleteAllEntries();
    expect(delRes.status).toBe(200);

    // 5. Documents cascades the tombstones on its next team-state sync; poll
