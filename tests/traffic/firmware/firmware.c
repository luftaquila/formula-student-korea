...
void usb_task(void){if(++iteration>(lose_clock_after_ready ? 12u : 7u))longjmp(done,1);}
...
int capture_sensor_get(uint64_t*t){if((iteration==2||iteration==5||iteration==6||(lose_clock_after_ready&&iteration==8))&&popped!=iteration){popped=iteration;*t=(uint64_t)(iteration*1000u+100u)*TICKS_PER_MS;return 1;}return 0;}
...
int main(int argc, char **argv) {
    ...
    if (lose_clock_after_ready) {
        return observed_drop > 0 && !(observed_flags & HEALTH_CAPTURE_OK) ? 0 : 1;
    }
