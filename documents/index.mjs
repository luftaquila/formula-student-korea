    ...
    const pending = detectTextCharset(filePath)
      .then((charset) => {
        try {
          ...
          logger.warn(null, "file.charset_cache", { error: e.message, file_id: file.id, file: file.original_name });
        }
        return charset;
      })
      .catch(() => "utf-8");
...
app.get("/api/submissions/:subId/files/:fileId", async (req, res) => {
  ...
  if (sub.team_num !== team.team_num) { logger.warn(req, "file.download", { error: "wrong_team", year: sub.session_year, sub_team: sub.team_num, my_team: team.team_num }, `#${sub.team_num}`); return res.status(403).send("권한이 없습니다."); }
  ...
  if (!isTarget) { logger.warn(req, "file.download", { error: "not_target", year: sub.session_year, session_id: sub.session_id }, `#${sub.team_num}`); return res.status(403).send("권한이 없습니다."); }
  ...
  if (isInitialDownload(req)) logger.log(req, "file.download", { session_name: session?.name, year: sub.session_year, team_num: sub.team_num, file: file.original_name }, `#${sub.team_num}`);
...
app.get("/api/submissions/:subId/zip", async (req, res) => {
  ...
  if (sub.team_num !== team.team_num) { logger.warn(req, "file.zip", { error: "wrong_team", year: sub.session_year, sub_team: sub.team_num, my_team: team.team_num }, `#${sub.team_num}`); return res.status(403).send("권한이 없습니다."); }

  // 해당 submission의 세션이 학생 팀에 할당된 세션인지 검증
  const isTarget = db.prepare("SELECT 1 FROM session_team st JOIN session s ON s.id = st.session_id WHERE st.session_id = ? AND st.team_num = ? AND s.year = ?").get(sub.session_id, team.team_num, team.year);
  if (!isTarget) { logger.warn(req, "file.zip", { error: "not_target", year: sub.session_year, session_id: sub.session_id }, `#${sub.team_num}`); return res.status(403).send("권한이 없습니다."); }
  ...
  logger.log(req, "file.zip", { session_name: session?.name, year: sub.session_year, team_num: sub.team_num, files: files.length }, `#${sub.team_num}`);
});
...
app.get("/api/admin/sessions/:id/archive", async (req, res) => {
  ...
  await archive.finalize();
  logger.log(req, "session.archive", { session_name: session.name, teams: subs.length, files: archiveFiles.length }, session.name);
});
...
app.get("/api/admin/submissions/:subId/files/:fileId", async (req, res) => {
  ...
  if (isInitialDownload(req)) logger.log(req, "file.admin_download", { session_name: session?.name, year: session.year, team_num: sub.team_num, file: file.original_name }, `#${sub.team_num}`);
  await setFileResponseHeaders(res, file, filePath);
  res.sendFile(filePath);
});
...
app.get("/api/admin/students", async (req, res) => {
  try {
    ...
    if (!authRes.ok) { logger.warn(req, "auth.fetch", { error: "auth fetch non-ok", status: authRes.status, body: await authRes.text().catch(() => "") }); return res.status(502).send("계정 서비스 연결 실패"); }
