...
app.get("/api/events", (req, res) => {
  ...
  if (!result.success) {
    logger.warn(req, "event.list", { error: result.error, cause: result.cause });
...
app.post("/api/events", (req, res) => {
  ...
  if (!result.success) {
    logger.warn(req, "event.create", { error: result.error, cause: result.cause, title, start, end });
    return res.status(result.status).json({ error: result.error });
  }
...
app.put("/api/events/:id", (req, res) => {
  ...
  if (!result.success) {
    logger.warn(req, "event.update", { error: result.error, cause: result.cause, title, start, end }, id);
    return res.status(result.status).json({ error: result.error });
  }
...
app.delete("/api/events/:id", (req, res) => {
  ...
  if (!result.success) {
    logger.warn(req, "event.delete", { error: result.error, cause: result.cause }, id);
    return res.status(result.status).json({ error: result.error });
  }
...
app.get("/api/events/ical", (req, res) => {
  ...
  if (!result.success) {
    logger.warn(req, "event.ical", { error: result.error, cause: result.cause });
