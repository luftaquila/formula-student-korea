...
app.get("/api/callback", async (req, res) => {
  ...
  try {
    ...
    if (name && name !== user.name) {
      ...
      if (!r.success) logger.warn(req, "user.name_sync", { error: r.error }, email, { email, name });
    }

    // 최초 로그인 시 created_at 기록
    if (!user.created_at) {
      const r = dbRun(() => db.prepare("UPDATE users SET created_at = datetime('now') WHERE id = ?").run(user.id));
      if (!r.success) logger.warn(req, "user.created_at_init", { error: r.error }, email, { email, name });
    }
