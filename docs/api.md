...
세션 객체: `{ event_type, armed, light_color, green_tick, master_boot_id, run_id, armed_at, team, event_name, controller, lease_expires_at, updated_at }`.

| Method | Path | Role | Request | Response | Description |
|--------|------|------|---------|----------|-------------|
| POST | `/wireless/clock` | `traffic.operate` | `{ request_id(32자 소문자 hex), master_tick(str 64-bit), master_boot_id(uint32) }` | `{ ok: true }` | `wireless:command { action: "clock", request_id }`에 대한 브리지 응답. 마스터의 `T` 응답을 전달한다. 잘못된 형식은 400, 만료·중복·미요청 응답은 409. |
| POST | `/wireless/ingest` | `traffic.operate` | `{ events?: [{ node_id, master_tick(str 64-bit), master_boot_id(uint32), ev_seq, flags?, rssi?, snr?, link_state? }], telemetry?: [{ node_id, rssi?, snr?, offset_us?, skew_ppm?, latency_ms?, link_state?, rx_miss?, beacon_gap?, temp_c10?, batt_mv?, sec_drop?, provisioned?, sync_valid?, skew_valid?, clock_source?, sync_age_ms?, capture_overflow?, event_drop?, queue_depth?, queue_overflow?, usb_ref_valid?, usb_ref_ppm? }] }` (각 ≤200) | `{ stored, deduped, rejected, acknowledged: [{ node_id, ev_seq, master_tick, master_boot_id }] }` | 브리지 배치 ingest. `(node_id, ev_seq, master_tick, master_boot_id)` 멱등. 원본 이벤트·진행 상태·공식 기록을 한 트랜잭션으로 저장한다. 저장 실패는 500이며 ACK·SSE를 보류하고 동일 이벤트 재시도를 허용한다. `acknowledged`는 트랜잭션 commit 또는 기존 행 dedupe가 확인된 정확한 키만 포함하며, 브리지는 이 키를 USB `C` ACK로 돌려 마스터 RAM 큐를 비운다. 불량 항목은 배치 전체 실패 없이 skip. 상태 악화는 armed 세션을 중단하고 `wireless.quality_fault`를 기록한다. 보안 관측은 `wireless.security` 로그로 남긴다. |
| POST | `/wireless/light` | `traffic.operate` | `{ color: red\|green\|yellow\|off, green_tick?(str), master_boot_id?(uint32) }` | `{ ...light }` | 브리지가 물리 신호등 색 보고. green은 master tick·boot ID가 필수이며 해당 시점 팀·이벤트명을 런에 고정한다. 새 green 보고도 해당 경기 품질 게이트를 통과해야 하며, 거부되면 409이고 브리지는 마스터에 OFF를 보낸다. |
...
| GET | `/wireless/events` | `traffic.operate` | `?since=<id>&limit=<n≤1000>` | `[{ id, node_id, master_tick, master_boot_id, ev_seq, server_time, rssi, snr, link_state }]` | 늦게 합류한 클라이언트의 raw 이벤트 백필 |
| POST | `/wireless/arm` | `traffic.operate` | `{ event_type, action: green\|red\|off\|reset, green_tick?(str) }` | `{ ...session }` | 경기 arm/disarm. 이미 armed인 경기의 green은 시각 요청 전에 409로 거부하며, 정지 또는 초기화 후 다시 시작해야 한다. green은 클라이언트 tick 대신 브리지·마스터와 `T` 왕복으로 확인한 tick·boot ID를 사용하며 시각 확인 실패는 409이다. 응답 session에는 `master_boot_id`가 포함된다. green은 필수 역할 센서 전부의 최신 link/provisioned/HFXO/sync/skew/beacon/capture/delivery 상태와 마스터 큐가 정상일 때만 허용(아니면 409). lease·reset 규칙은 동일하다. |
...
무선 펌웨어 2.2.0(무선 프로토콜 v8)은 마스터·센서 전체와 브리지를 함께 갱신해야 한다. USB `E`·`L` 및 `C` ACK에는 마스터 boot ID가 포함된다. 서버는 런의 boot ID와 일치하고 시작 경계 이상인 tick만 계산에 사용한다. 가속·오토크로스는 도착이 먼저 수신되어도 출발과 결합한다. 진행 상태에는 해당 런의 마스터·센서 진단과 원래 수신 시각을 함께 저장해 재시작 후 복구한다. 진단의 신선도는 원래 시각으로 판단하며, 재시작 후 전체 진단보다 대기 이벤트가 먼저 도착해도 아직 유효한 진단으로 처리한다. 만료된 진단이나 새로 보고된 실제 품질 오류는 기존대로 런을 중단한다. 이전 버전에서 진행 중이던 런은 복구 상태가 없으므로 업그레이드 시 중단하고 다시 arm해야 한다. 주파수 교정은 이 프로토콜 변경에 포함되지 않는다. 판정 변경과 런 완료 상태는 함께 저장하므로 정상 판정으로 복원해도 완료된 내구 런이 재시작 후 재개되지 않는다. 성공한 정지·초기화는 진행 중인 arm 시각 확인을 무효화한다. 브라우저는 arm 확정 전에 받은 최근 이벤트를 보관하고 일치하는 런에서 한 번만 처리한다.
