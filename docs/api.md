...
| POST | `/sheet/template/import` | `inspection.manage` | `{ year, template: [...] }` | 201 | Destructively import a full template from JSON; references exported from another year follow their stable keys into this year's catalog and stay `verified` only when the clause content is unchanged |
