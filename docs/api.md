...
| POST | `/wireless/arm` | `traffic.operate` | `{ event_type, action: green\|red\|off\|reset, green_tick?(str) }` | `{ ...session }` | 경기 arm/disarm. green은 클라이언트 tick 대신 브리지·마스터와 `T` 왕복으로 확인한 tick·boot ID를 사용하며 시각 확인 실패는 409이다. 응답 session에는 `master_boot_id`가 포함된다. green은 필수 역할 센서 전부의 최신 link/provisioned/HFXO/sync/skew/beacon/capture/delivery 상태와 마스터 큐가 정상일 때만 허용(아니면 409). lease·reset 규칙은 동일하다. |
