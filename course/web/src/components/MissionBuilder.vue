      ...
      <div class="builder-workspace">
        <section class="cone-picker">
          ...
          <div class="bulk-action-grid">
            ...
            <button class="danger-text full-row" @click="clearRoute">선택 비우기</button>
          </div>
          <div class="cone-list">
            ...
            >
              ...
              <span class="route-description">
                ...
                <span class="route-badges">
                  <em v-if="duplicateIds.has(item.cone_id)" class="repeat">반복 방문</em>
                  <em v-if="uncertainIds.has(item.waypoint_id || item.id)" class="uncertain">결과 확인 필요</em>
                </span>
              </span>
...
.mission-builder input,
.mission-builder select { width: 100%; padding: .45rem .65rem; }
...
.preset-row {
  flex: 0 0 auto;
  display: grid;
  grid-template-columns: minmax(180px, 1fr) minmax(170px, 1fr) 76px 76px;
  gap: .55rem;
...
.map-action-grid button { padding-inline: .35rem; font-size: .78rem; }
...
.route-row {
  ...
  min-height: 76px;
  display: grid;
  grid-template-columns: 20px 57px 12px minmax(0, 1fr) 84px;
  align-items: center;
  gap: .42rem;
  padding: .45rem .5rem;
  border-bottom: 1px solid rgba(148, 163, 184, .13);
...
.drag { color: #94a3b8; font-size: 1rem; cursor: grab; user-select: none; }
.route-position { display: grid; grid-template-columns: 1fr; justify-items: center; gap: .08rem; }
...
.route-position small { color: #94a3b8; font-size: .62rem; }
.route-badges { display: flex; flex-wrap: wrap; gap: .2rem; margin-top: .24rem; }
.route-badges em {
  padding: .08rem .28rem;
  border-radius: 999px;
  font-size: .62rem;
...
.footer-actions button { min-width: 92px; }
.footer-actions .primary { min-width: 138px; }
...
@media (max-width: 1320px) {
  ...
  .route-row { grid-template-columns: 20px 57px 12px minmax(0, 1fr) 172px; }
  .route-row-actions { grid-template-columns: repeat(4, 40px); }
}
...
@media (max-width: 760px) {
  ...
  .route-row { grid-template-columns: 18px 54px 11px minmax(0, 1fr) 84px; }
  .route-row-actions { grid-template-columns: repeat(2, 40px); }
...
@media (max-width: 420px) {
  ...
  .route-row { grid-template-columns: 16px 50px 10px minmax(0, 1fr) 76px; gap: .3rem; padding-inline: .35rem; }
  .route-position .position { width: 48px; }
  .route-row-actions { grid-template-columns: repeat(2, 36px); }
  .mission-builder .route-row-actions button { width: 36px; min-height: 36px; }
